...
Given stable environmental contingencies, it is adaptive for an organism to develop routine ways of performing tasks requiring multiple responses. Dopamine is assumed to play a key role in the neural computations that underlie the formation of task routines. A large body of evidence shows that dopaminergic midbrain neurons encode reward prediction errors, a teaching signal assumed to drive the selection of actions in order to maximise reward. A comparable signal, also assumed to be modulated by dopamine release, marks the difference between the expected and actual saccadic sequence length used by macaques to attain reward during a free-viewing visual search task. This signal is assumed to reflect a cost-benefit signal that can drive routine learning. There also exists a large body of evidence from rodent and macaque models suggesting that increased striatal dopamine availability speeds the transition from goal-directed to habitual control of behaviour, the latter of which is assumed to govern performance of routines [Defouli?]. Although this evidence implicates dopamine in the formation of task-relevant routines, whether dopamine availability modulates the formation of behavioural routines in healthy humans remains an open question. 

In humans, elevation of dopamine levels by administration of L-Dopa, a precursor to dopamine, has been associated with elevated positive prediction errors in striatal blood-oxygenation-level-dependent BOLD responses [@pessiglioneDopaminedependentPredictionErrors2006] as well as increased tendencies to exploit previous actions [] during instrumental learning tasks. This suggests that L-Dopa induces artificially optimistic evaluation of outcomes [@fitzgeraldDopamineRewardLearning2015]. Elevating dopamine availability may carry a comparable impact on the cost-benefit computations that drive the formation of eye-movement routines under free-viewing visual search conditions, by promoting an overly optimistic evaluation of the energetic costs and benefits of the performed sequence, thereby increasing the probability that it will be adopted as a routine. 

For adopted task-oriented routines to be adaptive, it is also required that they are not confused between tasks, despite overlap in the situational cues that mark task environments. Dopamine availability is also assumed to play a modulatory role in the activation of task-relevant behaviours in response to relevant situational cues. Patients with Parkinson's Disease consistently show deficits switching between simple stimulus-response tasks, as do healthy participants who have been administered D2 antagonists [robbins]. Such findings have been theoretically accounted for by assuming that decreased dopamine increases uncertainty when updating the likelihood of being in a specific task-state, given sensory evidence [Friston]. These assumptions are based on evidence from constrained tasks - i.e. when a single correct response is required for a given stimulus. In contrast, eye-movement routines are often formed from a self-selected set of many possible eye-movements. 

As with the formation of task-relevant routines, the quantitative relationship between dopamine availability and the ability to switch between self-chosen and unconstrained routines is currently unknown for healthy human participants. Increase DA = greater reliance on model free, or what was done on the previous trial . Increase DA = increased precision of task state encoding, therefore better switching.
...
A further but less frequently discussed component of the processes underlying task-relevant routine learning and deployment, is the brain's representation of task-relevant cues and actions, presumably encoded by cortex and relayed to striatum for reinforcement [cite Killcross, Ashby, Bar-Gaid, something on credit assignment]. Presumably, the organism that encodes an accurate representation of cues, actions and outcomes is at an adaptive advantage when forming and deploying task-relevant routines. A growing body of empirical evidence suggests that mindfulness may modulate fidelity of task-representations. Theoretically, mindfulness is defined as a mental state of not being preoccupied with events other than current sensory and internal inputs, and as such is well-placed to promote accurate task-representations. In support of this notion, greater mindfulness has been associated with increased error monitoring during cognitively challenging tasks [Heleen], and with greater sensitivity to dynamics in operant reinforcement contingencies []. This greater sensitivity to task contingencies suggests that mindfulness promotes a more precise representation of the task state. Indeed, the computational notion of uncertainty used to explain the function of tonic dopamine in task-switching has also been used to account for the impact of mindfulness on information processing; mindfulness reduces uncertainty regarding task-relevant sensory signals, increasing their priority over non-sensory or non-relevant signals. 
Mindfulness on +DA and learning/routine. Mindfulness on +DA and task-switching.
Specific hyp.
