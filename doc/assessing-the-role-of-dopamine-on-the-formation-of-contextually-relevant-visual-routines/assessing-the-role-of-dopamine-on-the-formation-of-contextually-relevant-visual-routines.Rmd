...
Given stable environmental contingencies, it is adaptive for an organism to develop routine ways of performing tasks requiring multiple responses. Dopamine signalling is assumed to play a key role in the neural computations that underlie the formation of task-relevant routines. A large body of evidence shows that dopaminergic midbrain neurons encode reward prediction errors, a teaching signal assumed to drive the selection of actions in relation to their success in maximising reward. A comparable signal, assumed also to be modulated by dopamine release, marks the difference between the expected and actual effort required for macaques to attain reward under a free-viewing visual search task. This signal is assumed to reflect a cost-benefit signal for an eye-movement sequence, and has been forwarded as a teaching signal for routine learning. There also exists a large body of evidence from rodent and macaque models suggesting that increased dopamine availability speeds the transition from goal-directed to habitual behaviour, the latter of which is assumed to underpin behavioural routines [Defouli?]. Although this evidence implicates dopamine in the formation of behavioural routines, whether dopamine availability modulates the formation of behavioural routines in healthy humans remains an open question. 

For behavioural routines to be adaptive, it is also required that the correct routine is implemented for the current task, despite common co-occurrence of multiple situational features between tasks. Dopamine signalling is assumed to play a modulatory role in the activation of task-relevant behavioural repertoires on the basis of relevant situational cues. Patients with Parkinson's Disease consistently show deficits switching between simple stimulus-response tasks, as do healthy participants who have been administered L-Dopa, a dopamine precursor that increases dopamine availability. Such findings have been theoretically accounted for by assuming that tonic dopamine levels signal uncertainty regarding the likelihood of being in a specific task-state, given sensory inputs [Friston]. These assumptions are based on evidence from constrained tasks - i.e. a single correct response is required for a given stimulus. In contrast, routine behaviours are often formed from a self-selected set of many possible behaviours. As with the formation of task-relevant routines, the quantitative mapping between dopamine availability and the ability to switch between task-relevant routines is currently unknown for healthy human participants.

A further but less frequently discussed component of the processes underlying routine learning and deployment, is the brain's representation of task-relevant cues and actions, presumably encoded by cortex and relayed to striatum for reinforcement [cite Killcross, Ashby, Bar-Gaid]. Presumably, the organism that encodes an accurate representation of recent cues, actions and outcomes is at an adaptive advantage. Empirical evidence suggests that mindfulness may a modulator of task-representations. Mindfulness is defined as a mental state of not being preoccupied with events other than current sensory and internal inputs, and as such is well-placed to promote accurate task-representations. In support of this notion, greater mindfulness has been associated with increased error monitoring during cognitively challenging tasks [Heleen], and with greater sensitivity to dynamics in operant reinforcement contingencies []. This suggests that greater mindfulness corresponds to []. Theoretically and within the same framework used to explain the function of tonic dopamine on task-switching, mindfulness has been linked to the appropriate weighting of relevant sensory signals, over non-sensory or non-relevant signals, for the current task-state. How dopamine and mindfulness act together to influence task-relevant routine formation is yet to be characterised.
