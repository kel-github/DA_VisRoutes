...
sum_dat$qs[sum_dat$m.x > qs[2]] <- 2
sum_dat$qs <- as.factor(sum_dat$qs)
sum_dat$pred_v <- exp(sum_dat$pred_v)
...
As both overall accuracy and stereotypy were found to be influenced by mindfulness and drug session, we conducted an exploratory analysis to characterise the relationship between accuracy and stereotypy. Although accuracy and stereotypy showed opposing relationships with mindfulness and drug - higher mindfulness scores were associated with a deleterious impact of DA on accuracy and a concomitant increase in stereotypy - we sought to determine whether these opposing effects could be attributed to the same underlying factor, such as an overall accuracy/stereotypy trade-off. A correlation analysis on the mean accuracy and stereotypy data did not lend support to this possibility, instead accuracy and stereotypy were positively correlated (insert test here). This suggests that the opposing pattern of results cannot be simply attributed to a general accuracy/stereotypy trade off in the data. To further rule this out, we added accuracy scores as a regressor to the winning stereotypy model. Adding accuracy scores served to increase the estimated drug x mindfulness interaction ()
