...
We investigated the impact of trait mindfulness and dopamine availability on the learning of task-relevant behaviour sets, and on the stereotypy of their deployment. Participants used their eyes to open doors in a gaze-contingent display, the colour of which signalled likely target locations, therefore each colour represented the current task-set. We assessed how well participants learned all the locations regardless of task-set (accuracy), the routine nature of sets of door selections (stereotypy), and how suited door selections were for the given task-set (setting-accuracy). Trait mindfulness and L-Dopa interacted to influence accuracy and stereotypy, whereas only L-Dopa modulated setting-accuracy. Whether the impact of L-Dopa on accuracy was positive or negative was dependent on trait mindfulness; L-Dopa was more likely to have a positive impact on accuracy for high trait mindfulness scorers (and a negative impact for low scorers). The opposite pattern was found for stereotypy; L-Dopa was more likely to decrease stereotypy for high mindfulness scorers, and increase stereotypy in low mindfulness scorers. In contrast, we found that L-Dopa reduced setting-accuracy independent to trait mindfulness. These results quantify, for the first time, that increasing systemic dopamine availability induces a trade-off between accuracy and stereotypy that is modulated by trait-behavioural qualities of mindfulness, and that increasing dopamine availability increases the likelihood of behavioural confusion between task-settings. These findings carry implications for our theoretical understanding of how the brain establishes and switches between task-relevant behavioural routines, which we outline below.

What could cause mindfulness to modulate the impact of dopamine availability on accuracy and stereotypy? We hypothesise that mindfulness may impact the quality of task-behaviour representations that are relayed between the cortex and striatum during the computation of incentive salience [e.g. @berridgeWhatRoleDopamine1998, @bar-gadReinforcementdrivenDimensionalityReduction2000a; beyelerNeuralCorrelatesSparse2019a]. The consequence of lower mindfulness is lower fidelity representations, that combined with increased reinforcing signals results in entrenchment of an impoverished, and somewhat superstitious, model of the actions that led to successful outcomes. This interpretation accords with previous separate theoretical accounts of the functions underlying mindfulness, and the nature of striatal-cortical interactions; higher mindfulness has been associated with maintenance of information pertaining to the present moment [@laukkonenManyOneMeditation2021a] at the expense of non-present information, such as rumination on previous experiences [@bucknerSelfprojectionBrain2007]. Furthermore, it has been proposed that the relay of information from cortex to striatum reflects a process of reinforcement-driven dimensionality-reduction [@bar-gadReinforcementdrivenDimensionalityReduction2000a; beyelerNeuralCorrelatesSparse2019a], that entrenches information regarding what behaviours led to rewards. If low mindfulness causes current events to be represented with low fidelity, this would induce greater variability in the information that is relayed from  cortex to striatum, inducing uncertainty regarding the exact actions taken, or their order. Under conditions of increased dopamine availability, such an impoverished model may be vulnerable to being tagged as a representation that corresponded to reward attainment. This could promote the formation of suboptimal policies, thereby decreasing accuracy but increasing routine, or repetition of sets of actions. 

This interpretation accords with recent theoretical assertions that working memory, a construct related to the current contents of mental experience, is a critical component of reinforcement learning [@yooHowWorkingMemory2022], and with recent findings that mindfulness interventions may act to promote less variable neural responses to the attainment of reward, as assessed by changes in the BOLD signal [@kirkShorttermMindfulnessPractice2019]. Specifically, individuals participating in a mindfulness intervention programme show attenuated BOLD responses in the putamen, relative to controls, when reward delivery violates expectations. The same group also shows increased BOLD responses in cortical regions assumed to encode stimulus driven responses, invariant to stimulus probability. One interpretation of these findings is that mindfulness motivates a steadfast encoding of the events that led to successful outcomes, that is less impacted by variability in stimulus and reward information.

This account can also inform our understanding of how the brain develops task-relevant routines, particularly when considered in conjunction with findings from non-human primate studies. The output activity of dopaminergic cells in macaque caudate nucleus have been shown to modulate eye-movements towards reward-related stimuli [@kunimatsuCaudalPartPutamen2019; @amitaOptogeneticManipulationValuecoding2020b; @nakamuraRoleDopaminePrimate2006], and neural activity in this region is sensitive to the cost of stereotypical eye-movement sequences in the macaque brain [@desrochersOptimalHabitsCan2010, @desrochersHabitLearningNaive2015], i.e. how long a sequence was relative to the expected sequence used to attain reward. If the consequence of low mindfulness is the relay of imprecise representations of current behaviours from cortex to striatum, this would presumably challenge the computation of cost-functions used to guide effort made in pursuit of rewards, motivating convergence to a suboptimal strategy. These results therefore expand our understanding of how dopamine signalling may drive habitual behavioural routines, by demonstrating the importance of behaviour-trait qualities that pertain to awareness of the current moment. 

Interestingly, increased dopamine availability reduced setting-accuracy, and this impact was not modulated by trait mindfulness. Therefore, increasing dopamine availability increases the probability that a behaviour from alternate task-sets are more competitive for control during performance of the task. These findings complement others showing that ingestion of a D2/D3 agonist promotes task-switching impairments [@mehtaImpairedSetshiftingDissociable2004]. The current data suggest that L-Dopa either acted to reduce certainty over what is the current relevant task-set (i.e. a task-set switch), or induced confusion regarding which locations belong to each task-set. These interpretations fit with previous work showing that the output of dopaminergic neurons from songbird Area X may act to excite cortical representations of the range of behaviours that are appropriate for a given task [@budzilloDopaminergicModulationBasal2017]. It is, however, also possible, that dopamine availability prohibited the learning of each task-set, rather than causing setting confusion. Although previous work has dissociated D2/D3 agonist induced task-switching deficits from learning deficits [@mehtaImpairedSetshiftingDissociable2004], we shall seek to disambiguate between these possibilities in future work.

Our interpretation is based on the assumption that L-Dopa impacted phasic dopamine signalling in the striatum [@willuhnExcessiveCocaineUse2014], or up-weighted cortical-striatal synaptic plasticity via a paracrine effect [@albinMissingShortLong2017]. There are also theoretical frameworks that posit a role for tonic dopamine in the deployment of task-relevant sequences. It has been proposed that tonic dopamine signals the precision of an agent's internal model of actions and sensory consequences [@fitzgeraldDopamineRewardLearning2015; @fristonDopamineAffordanceActive2012], assuming a positive monotonic relationship between dopamine and precision. Mindfulness practice has also been associated to the modulation of comparable precision parameters [@laukkonenManyOneMeditation2021a]. Interpreted via this lens, the current findings suggest a nuanced relationship between variability in internal models and dopaminergic precision signals; it is possible that increased tonic dopamine can induce overestimates of precision, or that the relationship between tonic dopamine and precision signalling is not necessarily monotonic. Indeed, the finding that L-Dopa served to reduce setting-accuracy, regardless of mindfulness state, suggests that our results may not be explainable under the assumption of a monotonic relationship between tonic dopamine and precision signalling, as this account would predict that increasing dopamine would increase the precision of task-set representations. We observed that setting-accuracy reduced with dopamine availability, as L-Dopa increased the probability of door-selections from the wrong task-set. 
