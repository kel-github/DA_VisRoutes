---
title: Assessing the influence of dopamine and mindfulness on the formation of task-relevant eye-movement routines
authors:
  - name: Kelly G. Garner
  ...
  - name: Paul  E. Dux
    ...
    location: St. Lucia, QLD
abstract: |
  Given experience in cluttered but stable visual environments, our eye-movements adapt to form stereotyped routines that frequent the locations most likely to offer task-relevant information, while not confusing routines between similar task-environments. Theoretical insights and empirical data suggest that dopamine function and mindfulness may be important modulators of the formation and deployment of such routines, yet quantification of their impact remains to be performed in healthy humans. Over two sessions, participants observed a gaze contingent display comprised of a 4 x 4 grid of doors, and were instructed to use their eyes to open doors in order to find hidden targets. Within each session, doors appeared in either one of two colours, with colour signalling differing likely target locations (task-environment). We derived measures for how well participants learned the target locations (accuracy), how stereotypical was their deployment of eye-movement routines (stereotypy), and how well eye-movements disambiguated the task-environments (task-relevant accuracy). Participants received either Maladopar (dopamine precursor) or placebo (vitamin C) across the 2 sessions, administered under double-blind conditions. Dopamine and trait mindfulness (assessed by questionnaire) interacted to influence both accuracy and stereotypy. Increasing dopamine improved accuracy and reduced stereotypy for individuals scoring high for mindfulness, and induced the opposite pattern for low mindfulness scorers. Dopamine also disrupted task-switching across all levels of mindfulness. The data suggest that mindfulness modulates the impact of dopamine on the formation of stereotyped eye-movement routines; individuals scoring low for mindfulness may settle on suboptimal routines at the expense of learning. Increasing dopamine promotes confusion between task-environments, regardless of mindfulness state. These findings provide a link between non-human and human models regarding the influence of dopamine on the formation of task-relevant eye-movements, and provide novel insights into modulating factors when humans use experience to build adaptive repertoires.  
...
Given stable environmental contingencies, it is adaptive for an organism to develop routine ways of performing tasks requiring multiple responses. Dopamine is assumed to play a key role in the neural computations that underlie the formation of task routines. A large body of evidence shows that dopaminergic midbrain neurons encode reward prediction errors, a teaching signal assumed to compute the value of actions. A comparable signal in striatum marks the difference between expected and actual saccadic sequence lengths used by macaques to attain reward during a free-viewing visual search task. This signal is assumed to reflect a cost-benefit signal that computes the value of routines. There also exists a large body of evidence from rodent and macaque models suggesting that increased striatal dopamine availability speeds the transition from goal-directed to habitual control of behaviour, the latter of which is assumed to govern performance of routines [Defouli?]. Although this evidence implicates dopamine in the formation of task-relevant routines, whether dopamine availability modulates the formation of saccadic routines in healthy humans remains an open question. 

One way to address this question is to increase dopamine availability via administration of L-Dopa, a precursor to dopamine. L-Dopa administration in humans has been associated with elevated positive prediction errors in striatal blood-oxygenation-level-dependent BOLD responses [@pessiglioneDopaminedependentPredictionErrors2006] as well as increased tendencies to exploit previous actions [] during instrumental learning tasks. This suggests that L-Dopa may induce artificially optimistic evaluation of outcomes [@fitzgeraldDopamineRewardLearning2015], falsely increasing the perceived value of performed actions. Elevating dopamine availability via L-Dopa may therefore have a comparable impact on the cost-benefit computations driving the formation of eye-movement routines under free-viewing visual search conditions, by promoting an overly optimistic evaluation of the energetic costs and benefits of the performed sequence, thereby increasing the probability that it will be adopted as a routine. 

For adopted task-oriented routines to be adaptive, it is also required that they are not confused between tasks, despite overlap in the situational cues that mark task environments. Dopamine is assumed to play a modulatory role in the activation of task-relevant behaviours in response to relevant situational cues, as well as promoting the formation of routines. Patients with Parkinson's Disease consistently show deficits switching between simple sensorimotor tasks, as do healthy participants who have been administered D2 antagonists [robbins]. Such findings have been accounted for by assuming that decreased dopamine causes increased uncertainty about the probability of being in a specific task-state when updating likelihoods on the basis of sensory evidence [Friston]. These assumptions are based on evidence from constrained tasks - i.e. when a single correct response is required for a given stimulus. In contrast, eye-movement routines are often formed from a self-selected set of many possible eye-movements, and it is unclear whether dopamine modulates confusion - i.e. performing the wrong routine in response to sensory cues. It remains unclear whether increasing dopamine has an impact on routine confusion, and if an impact is observed, whether the effect is opposite to that of depleted dopamine, i.e. does increasing dopamine availability promote segregation of task-routines? Or, does the influence of dopamine follow an inverted U function, where both depleted and inflated dopamine levels result in greater confusion between task-relevant routines?  

https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3272648/
https://elifesciences.org/articles/51260#data
https://www.sciencedirect.com/science/article/pii/S1053811918320561?casa_token=zTehqhHdNqwAAAAA:UPucqDccpGiJ-N_2CJ2p9a4KilCvzar_osTvrmHhoyGHbkNAcQCudJgE6iW-7cPs2PdU8g8XE3s increased model-based - one for the increase and increased precision.

A further but less frequently discussed component of the processes underlying task-relevant routine learning and deployment, is the brain's representation of task-relevant cues and actions, presumably encoded by cortex and relayed to striatum for reinforcement [cite Killcross, Ashby, Bar-Gaid, something on credit assignment]. Presumably, the organism that encodes an accurate representation of cues, actions and outcomes is at an adaptive advantage when forming and deploying task-relevant routines. A growing body of empirical evidence suggests that mindfulness may modulate fidelity of such representations. Theoretically, mindfulness has been defined as a mental state of not being preoccupied with events other than current sensory and internal inputs, and as such is well-placed to promote accurate task-representations. In support of this notion, mindfulness practice has been associated with increased error monitoring during cognitively challenging tasks [Heleen], and with greater sensitivity to dynamics in operant reinforcement contingencies []. This suggests that increased mindfulness corresponds to a more precise representation of the stimulus and response elements that make up the task-state.  

What could be the modulatory influence of mindfulness on the formation and deployment of task-relevant routines? Mindfulness appears to have opposing influences to dopamine on routine learning and task-switching: individuals low in trait mindfulness are faster to exploit sequential regularities in stimulus-response tasks [], and exploitation of such regularities are assumed to support habitual responses [Df]. Mindfulness may also promote task-switching; higher levels of trait mindfulness has been shown to decrease reliance on past behaviours when stimuli are conserved across tasks that carry different cognitive demands [@greenbergMindTrapMindfulness2012; @kuoResetTaskSet2015]. Indeed both reinforcement learning (RL) and active inference frameworks have been used to posit common but opposing mechanistic actions for both mindfulness and dopamine. In the case of RL, mindfulness is assumed to attenuate striatal reward prediction errors, possibly regulated by stronger cortical representations of subjective values and internal states. In the case of active inference, both dopamine and mindfulness are assumed to modulate the estimate of uncertainty that is used to weight task-relevant prediction errors. These lines of evidence suggest that mindfulness may oppose the impact of elevated dopamine on task-relevant routine formation and deployment, yet it remains to be ascertained whether these assumed links are borne out in a quantitative test.

Thus, using a novel task designed to test the formation and deployment of task-relevant saccadic routines in humans, we sought to test whether elevating dopamine availability increased suboptimal routine formation, and whether increased dopamine positively or negatively modulated switching between routines. Last we sought to test whether higher levels of trait mindfulness offered a buffering effect against the impacts of increased dopamine availability. To preview the results, L-Dopa decreased accuracy and promoted routine formation in individuals with low trait-mindfulness, whereas high trait-mindfulness was associated with the opposite pattern. Regardless of mindfulness, dopamine hampered switching between routines by increasing routine confusion.  



role for mindfulness 

A common theoretical framework that has been used to account for the function of dopamine in task-switching and reward-learning, and the impact of mindfulness on task-relevant sensory processing. In the case of the latter, mindfulness is assumed to reduce uncertainty regarding 

computational notion of uncertainty used to explain the function of tonic dopamine in task-switching has also been used to account for the impact of mindfulness on sensory information processing; mindfulness reduces uncertainty regarding task-relevant sensory signals, increasing their priority over non-sensory or non-relevant signals. 

These theoretical and empirical insights suggest that both dopamine and mindfulness modulate the functions that underpin the formation and deployment of task-relevant routines, yet it remains an open question whether their quantitative impact can be observed in humans, and whether or not their impact is convergent. Using a novel task designed to assess the formation and deployment of task-relevant eye-movement routines in healthy humans, we sought to quantify the impact of administration of L-Dopa, a precursor to dopamine that increases dopamine availability, and to determine whether this impact was modulated by trait mindfulness. In line with previous findings that L-Dopa inflates positive reward prediction errors [nat paper], the mismatch signal for sequences - leading to a vulnerability to 


we hypothesised that L-Dopa would promote the crediting of actions as successful, leading to erroneous repetition of unsuccessful actions.     


In line with previous theory We hypothesised that increasing dopamine availability would 


Using a novel task designed to assess the formation and deployment of task-relevant eye-movement routines in humans, we sought to map the joint modulatory impact of increased dopamine availability and mindfulness. 
We expected a) that increased dopamine will cause hyper-crediting of events as successful, which is good for high mindfulness but bad for low mindfulness. b) Dopamine will cause an increased transition to habit - thus high mindfulness = transition to good routines, low mindfulness = transition to suboptimal routines, and c) that dopamine will create confusion over task contexts, which will be alleviated by mindfulness, owing to better encoding of sensory cues.
...
-- add bit here showing the response types plotted.
...
Discussion plan/adjusts -
1) summary
2) hypothesis re: acc, cacc, stereotype, what was supported but what should we consider
3) findings - computational underpinnings (reuben/friston or reinforcement learning framework)
4) neurobiological underpinnings - paracrine effect vs tonic dopamine vs phasic
5) caveats
6) conclusion

mechanisms of task switching difficulty - over-optimistic evaluation of previous strategy, or failure to perform a sensory update?

https://www.nature.com/articles/npp2014193
might have affected frontal dopamine
or the effect of L-Dopa may be influenced by the choice of task

 It has been shown that higher levels of dopamine transmission elevates the probability that cortical inputs trigger basal-ganglia action potentials that trigger action selection [], and mediates a more rapid shift to habitual styles of responding [], meaning X, Y & Z. 
Higher mindfulness increases sensitivity to dynamics in reinforcement contingencies [] as well as reduced variability in the cortical correlates assumed to underpin their representation []. Therefore, mindfulness may promote the transmission of a better model of the task environment via corticostriatal connections, resulting in the reinforcement of optimal task-relevant habits. This would suggest that X. 

We investigated the impact of trait mindfulness and dopamine availability on the learning of task-relevant behaviour sets, and on the stereotypy of their deployment. Participants opened doors to search for targets in a gaze-contingent display. The colour of the display signalled likely target locations, making some locations relevant for only that colour. We assessed how well participants learned all possible target locations (accuracy), how consistent was the order of door selections across trials (stereotypy), and how well participants learned to disambiguate between colour settings (setting-accuracy). The impact of L-Dopa on accuracy was dependent on trait mindfulness; L-Dopa promoted accuracy for high trait mindfulness scorers (and reduced accuracy for low scorers). The opposite pattern was found for stereotypy; L-Dopa reduced stereotypy for high mindfulness scorers, and promoted stereotypy in low mindfulness scorers. In contrast, we found that only L-Dopa reduced setting-accuracy, regardless of trait mindfulness. These results quantify, for the first time, that increasing systemic dopamine availability induces a trade-off between accuracy and stereotypy that is modulated by trait-mindfulness, and that increased dopamine availability impairs setting-accuracy. These findings carry implications for our theoretical understanding of how the brain establishes and switches between task-relevant behavioural routines, which we outline below.

What could cause mindfulness to modulate the impact of dopamine availability on accuracy and stereotypy? We hypothesise that mindfulness may impact the quality of task-behaviour representations that are relayed between the cortex and striatum during the computation of incentive salience [e.g. @berridgeWhatRoleDopamine1998, @bar-gadReinforcementdrivenDimensionalityReduction2000a; @beyelerNeuralCorrelatesSparse2019a]. Lower mindfulness has been associated with lower explicit awareness of the current contents of cognitive states [@sandved-smithComputationalPhenomenologyMental2021; @laukkonenManyOneMeditation2021a; @lutzEpistemicPragmaticValue2019; @@bucknerSelfprojectionBrain2007]; the consequence of which may be low fidelity representations of currently performed behaviours. Concurrently, L-Dopa may inflate phasic dopamine responses [@willuhnExcessiveCocaineUse2014], possibly elevating positive reinforcement signals. We propose that the consequence of this combination is reinforcement of an imprecise model of the behaviours that led to successful outcomes. This interpretation accords with previous theoretical accounts positing that the relay of information from cortex to striatum reflects a process of reinforcement-driven dimensionality-reduction [@bar-gadReinforcementdrivenDimensionalityReduction2000a; @beyelerNeuralCorrelatesSparse2019a], where the striatum compresses cortical information and tags statistically reliable patterns with incentive salience. 

This account can also inform our understanding of how the brain develops task-relevant routines, particularly when considered in conjunction with findings from non-human primate studies. The output activity of dopaminergic cells in macaque caudate nucleus have been shown to modulate eye-movements towards reward-related stimuli [@kunimatsuCaudalPartPutamen2019; @amitaOptogeneticManipulationValuecoding2020b; @nakamuraRoleDopaminePrimate2006], and neural activity in this region is sensitive to the cost of stereotypical eye-movement sequences in the macaque brain [@desrochersOptimalHabitsCan2010, @desrochersHabitLearningNaive2015], i.e. how long a sequence was relative to the expected sequence used to attain reward. If the consequence of low mindfulness is the relay of imprecise representations of current behaviours from cortex to striatum, this would challenge the computation of cost-functions used to guide effort expended in pursuit of rewards, motivating convergence to a suboptimal strategy. These results therefore expand our understanding of how dopamine signalling may drive habitual, and even superstitious, behavioural routines. Specifically, behaviour-trait qualities that modulate awareness of the current moment may impact the representational fidelity of recent behaviours, increasing vulnerability that superfluous actions are tagged as significant for successful outcomes. 

Interestingly, increased dopamine availability reduced setting-accuracy regardless of trait mindfulness levels. Therefore, increasing dopamine availability increases the probability that a behaviour from alternate task-sets are more competitive for control during performance of the task. These findings complement others showing that ingestion of a D2/D3 agonist promotes task-switching impairments [@mehtaImpairedSetshiftingDissociable2004], and extends them by showing that increased dopamine availability also impacts task-switching for more complex behavioural-sets. The current data suggest that L-Dopa either acted to reduce certainty regarding the current relevant task-set (i.e. a task-set switch), or induced confusion regarding which locations belonged to each task-set. It is, however, also possible, that dopamine availability prohibited the learning of each task-set, rather than causing setting confusion. Although previous work has dissociated D2/D3 agonist induced task-switching deficits from learning deficits [@mehtaImpairedSetshiftingDissociable2004], we shall seek to disambiguate between these possibilities in future work.

Our interpretation is based on the assumption that L-Dopa impacted phasic dopamine signalling in the striatum [@willuhnExcessiveCocaineUse2014], or up-weighted cortical-striatal synaptic plasticity via a paracrine effect [@albinMissingShortLong2017]. There are also theoretical frameworks that posit a role for tonic dopamine in the deployment of task-relevant sequences. It has been proposed that tonic dopamine signals the precision of an agent's internal model of actions and sensory consequences [@fitzgeraldDopamineRewardLearning2015; @fristonDopamineAffordanceActive2012], assuming a positive monotonic relationship between dopamine and precision, where precision is the inverse of variability. Mindfulness practice has also been associated to the positive modulation of comparable precision parameters [@laukkonenManyOneMeditation2021a]. According to this view, increasing both dopamine and mindfulness should lead to increased setting accuracy. The current findings suggest a more nuanced relationship between dopamine, mindfulness and variability of internal states. Indeed, L-Dopa served to reduce setting-accuracy, regardless of mindfulness state, yet acted in concert with mindfulness to promote overall accuracy. Assuming L-Dopa influenced tonic dopamine signals (but see @albinMissingShortLong2017), this would suggest that the influence of dopamine and mindfulness varies across levels of the task-hierarchy. Both acted in concert at the context free level, but their impact was respectively negative and non-present at context setting levels of the task representation.

Another possible interpretation of the data is that increased dopamine-availability increased 'greedy' or random exploration of doors [e.g. k-armed bandit]. However, as mindfulness and dopamine interacted to both increase accuracy and reduce stereotypy, it is unlikely that the current results can be attributed to a dopamine-induced increase in random exploration that scales with trait-mindfulness, as this would cause both accuracy and stereotypy to decrease in concert. The current set of findings instead suggests that something is changing in the brain across levels of mindfulness that impacts how dopamine influences the formation of behavioural policies for a given task-setting.

The current work is not without limitations. A difference was found in mean BVP between the L-Dopa and placebo sessions, suggesting more general physiological differences between the sessions. However, participants were not able to detect whether they had received L-Dopa or placebo above what would be expected by chance. Therefore, the physiological changes appeared to not be subjectively detectable, lowering the likelihood that they impacted the results. Note that although the power of our blinding test was lowered owing to missing data, the remaining N was comparable to sample sizes from previous investigations into the impact of dopaminergic pharmacological intervention on decision-making, that employed comparable blinding tests [@leowDopamineIncreasesAccuracy2023a; @pineDopamineTimeImpulsivity2010; @wunderlichDopamineEnhancesModelBased2012]. 

Although accuracy and stereotypy theoretically need not be correlated, we did find a moderate positive correlation between the two measures. Critically, the modulatory influence of mindfulness and dopamine on stereotypy was found to be larger after accounting for accuracy. Furthermore, accuracy and stereotypy were at antithesis to each other with regard to the demonstrated impacts of mindfulness and dopamine availability. Nonetheless, further work should be done to confirm the dissociable impact of dopamine and mindfulness on these two aspects of performance. We shall seek to achieve this in future studies by controlling task parameters to maintain accuracy, while examining modulations to stereotypy. 

This work paves the way for further questions regarding the interaction of dopamine availability and trait mindfulness in the formation of task-relevant routines. If our hypothesis, that mindfulness impacts the quality of the task model that is relayed to striatum, is correct, then the cortical underpinnings of this process should be delineated. It could be that cortical representational quality is poorer, or that the relay of information from cortex to striatum is impoverished. Furthermore, while L-Dopa acts to generally increase dopamine availability, further work should isolate the exact dopaminergic pathways that underpin the current findings. It has been shown in rats that direct and indirect pathway neurons of the dorsomedial striatum may respectively bias behaviour repetitions and switches [@nonomuraMonitoringUpdatingAction2018] - both are mechanisms that could potentially underpin the current findings. It would therefore be informative to repeat the current study with pharmacological manipulations targeted at D1 and D2 signalling respectively.

Conclusions - 150 (176 max)

Mechanisms section - mindfulness causing low precision on sensory context, combined with dopamine creating over optimistic evaluations of the outcome state [Fitzgerald] - does impact general accuracy, and stereotypy. Note this combination does not impact context switching, rather only the latter. Low mindulness = more random selections, and increased stereotypy - therefore, evidence of poorer learning, with greater adherence to a suboptimal routine. 

Task-set switching - dopamine selectively decreases accuracy, and increases context-innaccuracy (but not random). Greater probability of response selection across the two contexts or increased confusion over context.
