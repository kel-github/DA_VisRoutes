...
dir.create(sprintf('../data/derivatives/%s', this_mod_sv_nm), showWarnings=FALSE)
...
if (new){
  ...
  # now save!
  save.image(file = sprintf('../data/derivatives/%s/%s.Rda', this_mod_sv_nm, this_mod_sv_nm))
  
} else {
  
  load(file = sprintf('../data/derivatives/%s/%s.Rda', this_mod_sv_nm, this_mod_sv_nm))
}
...
if (verbal){
 ...
 # prior_summary(mnd)
  ...
  verbal_output(mnd, dir_name = this_mod_sv_nm)
