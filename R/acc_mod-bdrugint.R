...
if (new){
  ...
  b_drug_int <- brm(formula = tt | trials(td) ~  b*drug + (1|sub),
                data = acc_dat,
                prior = priors,
  ...
  # now save!
  save.image(file = '../data/derivatives/acc_mod-bdrugint/acc_mod-bdrugint.Rda')
  
} else {
  
  load(file = '../data/derivatives/acc_mod-bdrugint/acc_mod-bdrugint.Rda')
}

# info
prior_summary(b_drug_int)
