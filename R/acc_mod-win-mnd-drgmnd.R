...
if (new){
  ...
  bdrgmdrgmi <- brm(formula = tt | trials(td) ~ b + drug + m + drug:m + b:drug + (b:drug|sub),
                   data = acc_dat,
                   warmup = 2000, iter = 10000,
                   family = binomial,
                   save_pars = save_pars(all=TRUE)) # for model comparisons 
  bdrgmdrgmi <- add_criterion(bdrgmdrgmi, "loo",  moment_match=TRUE, reloo=TRUE)
...
if (verbal){
 ...
 # prior_summary(mnd)
  ...
  verbal_output(bdrgmdrgmi, dir_name = dir_name)
