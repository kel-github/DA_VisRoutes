...
###-------------------------------------------------------
## compute contextual accuracy for each sub
###-------------------------------------------------------
door_acc_sum <- door_acc_sum %>% mutate(cacc = cc/(cc+oc))

###-------------------------------------------------------
## PLOT, for each subject: sess x drug x block x cond
###-------------------------------------------------------
door_acc_sum %>% ggplot(aes(x=b, y=cacc, 
                            colour=drug,
                            shape=as.factor(cond),
                            group=interaction(drug, as.factor(cond)))) +
                   geom_line() + facet_wrap(~as.factor(sub))
ggsave(filename='../images/quick-plots/cacc_by_drug.pdf', plot=last_plot(),
...
door_acc_sum %>% ggplot(aes(x=b, y=cacc, 
                  ...
                  geom_line() + facet_wrap(~as.factor(sub))
ggsave(filename='../images/quick-plots/cacc_by_sess.pdf', plot=last_plot(),
       height = 35, width = 20, units = 'cm', dpi = 300)

# now plot a quick average by drug and session
door_acc_sum %>% group_by(drug, cond, b) %>%
                   summarise(mu = mean(cacc)) %>%
                   ggplot(aes(x=b, y=mu,
                              colour=drug,
                              shape=as.factor(cond),
                              group=interaction(drug, as.factor(cond)))) +
                   geom_line() 
ggsave(filename='../images/quick-plots/mu_cacc_by_drug.pdf', plot=last_plot(),
...
ggsave(filename='../images/quick-plots/mu_cacc_by_sess_by_cond_int.pdf', plot=last_plot(),
       height = 15, width = 15, units = 'cm', dpi = 300)
...
door_acc_sum %>% group_by(sub, drug, sess, b) %>%
                  summarise(cacc=mean(cacc)) %>%
                  ggplot(aes(x=b, y=cacc,
                        group=as.factor(sub),
                        colour=as.factor(sub))) +
                  geom_line() + facet_wrap(~drug*as.factor(sess))
ggsave(filename='../images/quick-plots/cacc_by_sub_by_sess_by_drug_int.pdf', plot=last_plot(),
