...
if (new){
  ...
  bisbdint <- brm(formula = tt | trials(td) ~ b + drug + bis + b:bis + drug:bis + b:drug:bis + (b:drug|sub),
                   data = acc_dat,
                   warmup = 2000, iter = 10000,
                   family = binomial,
                   save_pars = save_pars(all=TRUE)) # for model comparisons 
  bisbdint <- add_criterion(bisbdint, "loo",  moment_match=TRUE, reloo=TRUE)
...
if (verbal){
 ...
 # prior_summary(mnd)
  ...
  verbal_output(bisbdint, dir_name = dir_name)
}
