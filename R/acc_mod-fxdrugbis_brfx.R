...
if (new){
  ...
  ###-----------------------------------------------------
  # note we know this works for mnd, so will work for bis
  nsubs <- 40
  intercept = 0.5
  b <- scale(1:8)
  nb <- 8
  ...
  fxdrgbis_bsubrfx <- brm(formula = tt | trials(td) ~  drug + bis + (b|sub),
                            data = acc_dat,
                            warmup = 2000, iter = 10000,
                            family = binomial,
                            save_pars = save_pars(all=TRUE)) # for model comparisons 
  ...
  # now save!
  save.image(file = '../data/derivatives/acc_mod-fxdrgbis_bsubrfx/acc_mod-fxdrgbis-bsubrfx.Rda')
  
} else {
  
  load(file = '../data/derivatives/acc_mod-fxdrgbis_bsubrfx/acc_mod-fxdrgbis-bsubrfx.Rda')
}

if (verbal){
  # info
  prior_summary(fxdrgbis_bsubrfx)

  pdf(file='../data/derivatives/acc_mod-fxdrgbis_bsubrfx/ps_and_chains.pdf')
    plot(fxdrgbis_bsubrfx)
  dev.off()

  summary(fxdrgbis_bsubrfx)

  # now look at some posterior predictive checks
  pdf(file='../data/derivatives/acc_mod-fxdrgbis_bsubrfx/pp_check.pdf')
    pp_check(fxdrgbis_bsubrfx)
  dev.off()
...
# see end of section 22.2.1
  est <- coef(fxdrgbis_bsubrfx)$sub[, "Estimate", ] %>% as.data.frame() %>%
              mutate(sub = unique(acc_dat$sub))
  check_dat <- inner_join(acc_dat, est, by = "sub")

  check_dat <- rbind(check_dat %>% filter(drug == "levodopa") %>% 
                       mutate(log_odds = Intercept + b.y * b.x + bis.x*bis.y),
                       check_dat %>% filter(drug == "placebo") %>%
                       mutate(log_odds = Intercept + drugplacebo + b.y*b.x + bis.x*bis.y)) %>% 
                       mutate(p= 1/(1+exp(-log_odds))) %>%
                       mutate(obs = tt/td)

  pdf(file='../data/derivatives/acc_mod-fxdrgbis_bsubrfx/predobs_resid.pdf')
...
if(compare){
  ...
  fxdrgbis_bsubrfx <- add_criterion(fxdrgbis_bsubrfx, "loo", moment_match=TRUE, reloo=TRUE) #PK ok
  fxdrg_bsubrfx <- add_criterion(fxdrg_bsubrfx, "loo", moment_match=TRUE, reloo=TRUE)
  loo_compare(fxdrgbis_bsubrfx, fxdrg_bsubrfx, criterion="loo")
  # original model is better
#  elpd_diff se_diff
  # fxdrg_bsubrfx     0.0       0.0   
  # fxdrgbis_bsubrfx -1.8       0.7   
  save.image('../data/derivatives/acc_mod-fxdrgbis_bsubrfx/acc_mod-fxdrgbis-bsubrfx.Rda')
}