...
axis(side=2, at=seq(.45, .75, .05), tick=TRUE, labels=c(".45","","","","","",".75"), las=2) #, labels=c())
with(mu_bdrug_dat %>% filter(drug == "placebo"), 
                              arrows(x0 = b.x, 
                              y0 = obs - ci,
                              x1 = b.x,
...
#######################################
variables(fxbdrgint_bdrgsubrfx)
fxdrg_draws <- posterior_samples(fxbdrgint_bdrgsubrfx, pars="b_drugplacebo")
...
fxbdrg_draws <- posterior_samples(fxbdrgint_bdrgsubrfx, pars="b_b:drugplacebo")
